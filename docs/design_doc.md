...
- Зачастую в промышленности требуется перебрать несколько вариантов имеющихся труб/каналов  
  ...
  в адекватных рамках.
- Успехом будем считать возможность получения системы/алгоритма, который позволит   
  автоматизированным путём получать потери давления в каналах со сложной геометрией   
  без проведения численных расчётов.
